...
import { GET_BUSINESS, SAVE_BUSINESS, VIEW_BUSINESS } from '../graphql/queries/business';
...
export function useBusinessDetail(businessId: string | number | null | undefined) {
  ...
  const [viewBusiness] = useMutation(VIEW_BUSINESS);
  const [saveBusinessMutation] = useMutation(SAVE_BUSINESS);
