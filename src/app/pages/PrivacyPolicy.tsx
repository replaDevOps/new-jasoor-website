...
export const PrivacyPolicy = ({
  ...
  const extractPolicyContent = (policyValue: unknown): string => {
    ...
    if (
      ...
      "content" in policyValue
    ) {
      const content = (policyValue as { content?: unknown }).content;
  ...
  const dynamicSections = policies
    .map((policyItem: any, index: number) => {
      const content = extractPolicyContent(
        isAr ? policyItem.arabicPolicy : policyItem.policy,
      );
  ...
  const sections =
    dynamicSections.length > 0 ? dynamicSections : staticSections;

  return (
    <div
      className={`min-h-screen bg-gray-50 font-sans ${direction === "rtl" ? "rtl" : "ltr"}`}
      dir={direction}
    >
      {/* Hero */}
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            {sections.map((section, i) => (
              ...
              >
                <h2 className="text-lg font-black text-[#111827] mb-3 flex items-center gap-2 [overflow-wrap:anywhere]">
                ...
                {/\<[^>]*\>/.test(section.content) ? (
                  <div
                    className="text-gray-600 leading-relaxed [overflow-wrap:anywhere] [&_*]:[overflow-wrap:anywhere]"
                    dangerouslySetInnerHTML={{ __html: section.content }}
                  />
                ) : (
                  <p className="text-gray-600 leading-relaxed [overflow-wrap:anywhere]">
                    {section.content}
                  </p>
                )}
              </div>
