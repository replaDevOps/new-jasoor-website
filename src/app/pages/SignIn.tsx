...
import { LOGIN, REQUEST_LOGIN_OTP, VERIFY_LOGIN_OTP } from '../../../graphql/mutations/auth';
...
export const SignIn = ({ onNavigate }: { onNavigate: (page: string) => void }) => {
  ...
  const [requestLoginOTP]         = useMutation(REQUEST_LOGIN_OTP, { errorPolicy: 'all' });
  const [verifyLoginOTP]          = useMutation(VERIFY_LOGIN_OTP,  { errorPolicy: 'all' });
  ...
  const handleVerifyOTP = async () => {
    ...
    if (otp.length < 6) {
      toast.error(isAr ? 'أدخل رمز التحقق كاملاً' : 'Enter the full OTP code');
      return;
    }
    setLoading(true);
    try {
      const { data, errors: gqlErrors } = await verifyLoginOTP({
        variables: { email: otpEmail, otp },
      });
      if (gqlErrors?.length) {
        toast.error(isAr ? 'رمز التحقق غير صحيح أو منتهي الصلاحية' : 'Invalid or expired OTP');
        return;
      }
      const { token, refreshToken, user } = data.verifyLoginOTP;
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      let normalized = raw;
      if (!raw.includes('@')) {
        const digits = raw.replace(/^\+\d{1,4}/, '');
        const noLeadingZero = digits.startsWith('0') ? digits.slice(1) : digits;
        normalized = countryCode + noLeadingZero;
      } else {
        normalized = raw.toLowerCase();
      }
      const { data, errors: gqlErrors } = await requestLoginOTP({
        variables: { email: normalized, password },
      });
      if (gqlErrors?.length) {
        const msg = gqlErrors[0].message;
        const isCredentials = msg.includes('Invalid') || msg.includes('credentials') || msg.includes('password') || msg.includes('not found');
        if (isCredentials) {
      ...
      setStep('otp');
      toast.success(isAr ? 'تم إرسال رمز التحقق إلى بريدك الإلكتروني' : 'OTP sent to your email');
    } catch (err: unknown) {
        ...
        {step === 'otp' ? (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-[#111827] mb-2">{isAr ? 'تحقق من بريدك الإلكتروني' : 'Check your email'}</h2>
              <p className="text-gray-500 text-sm">{isAr ? `تم إرسال رمز التحقق إلى ${otpEmail}` : `OTP sent to ${otpEmail}`}</p>
            </div>
            <div className="flex justify-center gap-3" dir="ltr">
              {otpDigits.map((d, i) => (
                <input
                  ...
                  className="w-11 h-14 text-center text-xl font-bold border-2 border-gray-200 rounded-xl focus:border-[#008A66] focus:outline-none transition-colors"
                />
              ))}
            </div>
            <button
              ...
              disabled={loading}
              className="w-full bg-[#008A66] text-white font-bold py-3.5 rounded-xl hover:bg-[#007053] transition-all disabled:opacity-60"
            >
              {loading ? (isAr ? 'جارٍ التحقق...' : 'Verifying...') : (isAr ? 'تحقق' : 'Verify OTP')}
            </button>
            <button
              onClick={() => { setStep('credentials'); setOtpDigits(['','','','','','']); }}
              className="w-full text-gray-500 text-sm hover:text-[#008A66] transition-colors"
            >
              {isAr ? 'العودة' : 'Back'}
            </button>
          </div>
