...
import { GET_BUYER_INPROGRESS_DEALS, GET_SELLER_INPROGRESS_DEALS } from '../../../graphql/queries/dashboard';
...
export const DealsView = () => {
  ...
  // P6-FIX R-04: real deals data
  const { data: buyerData,  loading: buyerLoading  } = useQuery(GET_BUYER_INPROGRESS_DEALS,  { variables: { limit: 50, offset: 0 }, errorPolicy: 'all' });
  const { data: sellerData, loading: sellerLoading } = useQuery(GET_SELLER_INPROGRESS_DEALS, { variables: { limit: 50, offset: 0 }, errorPolicy: 'all' });

  const rawDeals = tab === 'buyer'
    ? (buyerData?.getBuyerInprogressDeals?.deals   ?? [])
    : (sellerData?.getSellerInprogressDeals?.deals ?? []);
