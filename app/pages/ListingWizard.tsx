...
export const ListingWizard = ({ mode = 'create', initialData, onClose, onSuccess, onCancel }: ListingWizardProps) => {
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1 && (!formData.sellingType || !formData.title || !formData.category)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const input = {
        ...
        profit: formData.profit ? parseFloat(String(formData.profit).replace(/,/g, '')) : 0,
        revenue: formData.revenue ? parseFloat(String(formData.revenue).replace(/,/g, '')) : 0,
        profitMargen: formData.profitMargin ? parseFloat(String(formData.profitMargin)) : undefined,
        ...
        isByTakbeer: formData.sellingType === 'selling_by_taqbeel',
    ...
    <div className={cn("space-y-8 animate-in fade-in duration-300", direction === 'rtl' ? "slide-in-from-left-4" : "slide-in-from-right-4")}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div 
            onClick={() => setFormData({...formData, sellingType: 'selling_by_acquiring'})}
            className={cn(
          ...
          <div 
            onClick={() => setFormData({...formData, sellingType: 'selling_by_taqbeel'})}
            className={cn(
              "border-2 rounded-2xl p-4 cursor-pointer transition-all hover:border-[#10B981]/50 relative flex flex-col gap-2 h-full",
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-6">
        ...
        <div className="col-span-1">
          ...
          >
            ...
            <option value="eastern">{language === 'ar' ? 'الشرقية' : 'Eastern'}</option>
            <option value="madinah">{language === 'ar' ? 'المدينة' : 'Madinah'}</option>
            <option value="asir">{language === 'ar' ? 'عسير' : 'Asir'}</option>
            <option value="qassim">{language === 'ar' ? 'القصيم' : 'Qassim'}</option>
          </select>
        </div>
        <div className="col-span-1">
          ...
          >
            ...
            <option value="riyadh_city">{language === 'ar' ? 'الرياض' : 'Riyadh'}</option>
            <option value="jeddah">{language === 'ar' ? 'جدة' : 'Jeddah'}</option>
            <option value="dammam">{language === 'ar' ? 'الدمام' : 'Dammam'}</option>
            <option value="khobar">{language === 'ar' ? 'الخبر' : 'Khobar'}</option>
            <option value="makkah_city">{language === 'ar' ? 'مكة' : 'Makkah'}</option>
            <option value="madinah_city">{language === 'ar' ? 'المدينة' : 'Madinah'}</option>
          </select>
        </div>
      ...
      <div className="space-y-8 pt-4 border-t border-gray-100">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button 
              onClick={() => setFormData({...formData, liabilities: [...formData.liabilities, {name: '', date: ''}]})}
              className="text-[#10B981] text-xs font-bold flex items-center gap-1 hover:bg-[#E6F3EF] px-3 py-1.5 rounded-lg transition-colors"
            >
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button 
              onClick={() => setFormData({...formData, inventory: [...formData.inventory, {name: ''}]})}
              className="text-[#10B981] text-xs font-bold flex items-center gap-1 hover:bg-[#E6F3EF] px-3 py-1.5 rounded-lg transition-colors"
            >
          ...
          <div className="space-y-2">
            {formData.inventory.map((item: { name: string; quantity: string; value: string; date: string }, idx: number) => (
               <div key={idx} className="flex gap-2">
                  <input 
                    type="text" 
                    value={item.name}
                    onChange={e => {
                       const newInv = [...formData.inventory]; newInv[idx].name = e.target.value; setFormData({...formData, inventory: newInv});
                    }}
                    placeholder={t.itemName}
                    className={cn(InputStyles, "py-2")}
                  />
                  <button onClick={() => {
                     const newInv = formData.inventory.filter((_: unknown, i: number) => i !== idx);
                     setFormData({...formData, inventory: newInv});
                  }} className="text-red-400 hover:bg-red-50 p-2 rounded-xl border border-transparent hover:border-red-100 transition-colors">
                     <Trash2 size={18} />
                  </button>
               </div>
            ))}
            {formData.inventory.length === 0 && <p className={cn("text-xs text-gray-400 italic", direction === 'rtl' ? "text-right" : "text-left")}>{t.noItems}</p>}
          </div>
        </div>
      </div>
    </div>
